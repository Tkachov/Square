...
#include "..\screens\the_forest.h"
#include "..\screens\the_dungeon.h"
#include <iostream>
#include <string>
#include <time.h>
#include <windows.h>
...
void Game::start(int w, int h, int bits, int surface_params) {
 ...
 current_screen = new Main_Menu(this, surface, &loader, &input);
 done = false;
}
...
void Game::open_screen(ScreenID sid, bool recreate) {
 ...
 if(!recreate && static_screens.count(sid)!=0) new_screen = static_screens[sid];
  ...
  switch(sid) {
   case s_MAIN_MENU: new_screen = new Main_Menu(this, surface, &loader, &input); break;
   case s_THE_FOREST: new_screen = new The_Forest(this, surface, &loader, &input); break;
   case s_THE_DUNGEON: new_screen = new The_Dungeon(this, surface, &loader, &input); break;
   case s_screens_count: break; //hate those warnings
  }
  if(new_screen!=prev) {
...
void Game::save(string fn) {
 ...
 fout << "-1\n"; //end of screen infos
 _state.save(fout);
 fout << int(current_screen?current_screen->ID():s_MAIN_MENU) << "\n";
 fout.close();
}
...
void Game::load(string fn) {
 ...
 fin >> s;
 sid = ScreenID(s);
 open_screen((static_screens.count(sid)==0?s_MAIN_MENU:sid));
 fin.close();
}
...
ScreenID Game::sid() { return (current_screen?current_screen->ID():s_MAIN_MENU); }

bool Game::portal_available(int s) {
 if(s<0 || s>=int(s_screens_count)) return false;
 return portal_available(ScreenID(s));
}

bool Game::portal_available(ScreenID s) {
 switch(s) {
  case s_MAIN_MENU: case s_screens_count: return false;
  case s_THE_FOREST: case s_THE_DUNGEON: return true;
 }
 return false;
}
