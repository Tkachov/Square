...
void load_string_table(string lang, string filename) {
 string path = "res/txt/"; path+=lang; path+="/"; path+=filename; path+=".txt";
 if(!file_exists(path)) {
  path = "res/txt/"; path+=DEFAULT_LANGUAGE; path+="/"; path+=filename; path+=".txt";
...
void load_unicode_table(string lang, string filename) {
 ...
 if(!file_exists(path)) Logger::err << "Default (" << DEFAULT_LANGUAGE << ") unicode-table not found.\n";
